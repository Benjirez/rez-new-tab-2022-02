...
var model1 = mongoose.model(
  "rez2_collection1",
  mongoose.Schema({ item: String })
); //todoModel
var model2 = mongoose.model(
  "rez2_collection2",
  mongoose.Schema({ item: String })
); //todoModel
//var data = [{item: 'get blue milk'}, {item: "re-attach 3PO's arm"}, {item: 'pickup power converters at toshi station'}];

var textsSch = mongoose.Schema({
  text: [{ name: String, keyword: String, body: String }],
});

var model3 = mongoose.model(
  "quicktext3",
  mongoose.Schema({ title: [], texts: [textsSch] })
);
...
function dig(o0) {
  ...
  if (Object.prototype.toString.call(o0) === "[object Object]") {
    if (o0.title) {
      textOut2 +=
        "<h2 style='color: salmon' class='toggler'; >" +
        o0.title.__cdata +
        "</h2><br>";
    }
    for (o1 in o0) {
    ...
    for (var o1 = 0; o1 < o0.length; o1++) {
      if (o0[o1].name) {
        textOut2 +=
          "<div class='toggler'><b style='color: khaki' style='display:none;'>" +
          o0[o1].name.__cdata +
          " </b>";
    ...
    var data = model3.find({}, function (err, data) {
      ...
      function treeMe(myData) {
        ...
        for (myGroup = 0; myGroup < myData.length; myGroup++) {
          text +=
            "<h1>" + (myGroup + 1) + ". " + myData[myGroup].title + "</h1>";
          var myTemplate;
          for (
            myTemplate = 0;
            myTemplate < myData[myGroup].texts[0].text.length;
            myTemplate++
          ) {
            text +=
              "<b>" +
              (myTemplate + 1) +
              ". </b>" +
              myData[myGroup].texts[0].text[myTemplate].name +
              " - ";
            text +=
              "<b>" +
              myData[myGroup].texts[0].text[myTemplate].keyword +
              "</b><br><br>";
            text +=
              "" + myData[myGroup].texts[0].text[myTemplate].body + "<br><br>";
          }
        }
      }
